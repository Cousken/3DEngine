...
std::unique_ptr<SystemClass> g_ApplicationHandle;
...
void SystemClass::CreateInstance()
{
	g_ApplicationHandle = std::make_unique<SystemClass>();
}

std::unique_ptr<SystemClass>& SystemClass::GetInstance()
{
	return g_ApplicationHandle;
}

HINSTANCE SystemClass::GetAppHandle()
{
	return m_hinstance;
}
...
void SystemClass::InitializeWindows(int& screenHeight, int& screenWidth)
{
	...
	wc.lpszMenuName = MAKEINTRESOURCEW(IDC_MY3DAPP);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT umessage, WPARAM wparam, LPARAM lparam)
{
	switch (umessage)
	{
		...
		case IDM_ABOUT:
		{
			DialogBox(SystemClass::GetInstance()->GetAppHandle(), MAKEINTRESOURCE(IDD_ABOUTBOX), hWnd, SystemClass::About);
			break; 
		}
