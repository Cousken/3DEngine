...
matrix BoneMatrixArray[MAX_BONE_MATRICES];
...
//--------------------------------------------------------------------------------------
// Helper function for SkinVert
//--------------------------------------------------------------------------------------
matrix FetchBoneTransform( uint iBone )
{
    return BoneMatrixArray[ iBone + 0 ];
}
...
SkinnedInfo SkinVert( VS_INPUT input )
{
	...
	//Bone[0]
	uint boneIndex = input.Bones.x;
	float weight = input.Weights.x;
	matrix m = FetchBoneTransform( boneIndex );
	output.Pos += weight * mul( Pos, m );
	output.Norm += weight * mul( Norm, (float3x3)m );
	output.Tan += weight * mul( Tan, (float3x3)m );
	
	//Bone[1]
	boneIndex = input.Bones.y;
	weight = input.Weights.y;
	m = FetchBoneTransform( boneIndex );
	output.Pos += weight * mul( Pos, m );
	output.Norm += weight * mul( Norm, (float3x3)m );
	output.Tan += weight * mul( Tan, (float3x3)m );
	
	//Bone[2]
	boneIndex = input.Bones.z;
...
VS_OUTPUT VS( VS_INPUT aInput )
{
    ...
    SkinnedInfo vSkinned = SkinVert( aInput );
    
    output.Pos = mul( vSkinned.Pos, World );
    output.Norm = mul( vSkinned.Norm, World );
    ...
    output.Reflection = normalize( mul( vSkinned.Tan, (float3x3)World ));
